...
def fake_hybrid_search_response() -> SearchResponse:
    return SearchResponse(
        query_id=UUID("00000000-0000-0000-0000-000000000001"),
        trace_id=UUID("00000000-0000-0000-0000-000000000002"),
        request_id="request-1",
...
def test_hybrid_search_post_works_without_admin_key(monkeypatch) -> None:
    monkeypatch.setattr(
        "app.api.v1.routes.search.search_service.run_search",
        lambda db, request, request_id=None: fake_hybrid_search_response(),
    )
    app.dependency_overrides[get_db] = fake_db
    try:
        response = client.post(
    ...
    assert payload["lexical_index_name"] == "aletheia-lexical-test"
    assert payload["vector_collection_name"] == "aletheia-vector-test"
