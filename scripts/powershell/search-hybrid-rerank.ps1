...
if (-not (Test-Path $Python)) {
    Write-Error "Missing services/api/.venv. Run the setup commands before hybrid rerank search."
}
...
try {
    ...
    $ArgsList = @(
        "-m", "app.cli.search_hybrid_rerank",
        $Query,
        "--top-k", "$TopK",
        "--bm25-candidate-k", "$Bm25CandidateK",
        "--dense-candidate-k", "$DenseCandidateK",
        "--hybrid-candidate-k", "$HybridCandidateK",
